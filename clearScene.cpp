...
void clearScene::InitTexture() {
    ...
    unsigned char* data1 = stbi_load("clear.bmp", &width, &height, &numberOfChannel, 0);
...
void clearScene::processKey(unsigned char key, int x, int y)
{
    switch (key)
    {
    case 'q':
    ...
    case 's':
        if (stage == 1) {
            ...
            scene* scene = GameManager.curScene;   ////현재 씬을 tmp에 넣고 지워줌
            GameManager.curScene = new menuScene;
...
void clearScene::Mouse(int button, int state, int x, int y)
{

}
void clearScene::MouseMotion(int x, int y)
{
